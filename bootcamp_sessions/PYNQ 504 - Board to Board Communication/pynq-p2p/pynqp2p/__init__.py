...
def register(ip, key):
    ...
    # Normalize the IP/URL - remove http:// or https:// prefix if present
    # The library will add it back when making requests
    normalized_ip = ip.replace('http://', '').replace('https://', '')

    with open(ip_filename, 'w+') as ip_file:
        ip_file.write(normalized_ip)
